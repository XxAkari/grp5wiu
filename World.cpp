...
World::World() {
	currentLevel = 0;
	credits = 0;
	creditsRewarded = 0; // defensive - avoids ever adding garbage to credits
};
...
void World::printCombatUI(std::string playerName, int pHp, int pAtk, int eHp, int eAtk, int creds, int turnCounter, std::string statusEffects) {
	std::cout << "The Jimbob Paradox" << std::endl;
	std::cout << "Credits: " << creds << "    Turn: " << turnCounter << std::endl;
	...
	if (!statusEffects.empty()) {
		...
		std::cout << statusEffects << std::endl;
	}
	std::cout << "Press [X] to attack the enemy" << std::endl;
}

void World::earnCredits(std::string enemyType) {
	if (enemyType == "Boss" || enemyType == "Obobjib" || enemyType == "Bobmij" || enemyType == "Jimbob") {
